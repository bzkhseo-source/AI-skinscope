...
| 나이·동년배 비교, 피부나이 | 나이·성별(선택 입력) 기반으로 동년배 평균과 비교. 피부나이는 나이를 입력한 경우에만 산출하며(신뢰도 낮은 경우 안내 문구로 대체), 나이를 입력하지 않으면 추정 없이 "실제 나이가 입력되지 않아 분석할 수 없음"으로 안내 |
...
| 이름 | 역할 |
|---|---|
| 조민경 | 팀장 — 기획 검토·확정, 진행 관리, 최종 결과보고서·발표자료 |
| 서경환 | 백엔드/AI 파이프라인 전체 구현(비전 분석·RAG·에이전트·나이/피부나이·챗봇 등), 통합 검증 |
| 오주연 | 프론트엔드 UI 리디자인, PWA 설치 경험 개선 |
| 강하연 | 실사용자 테스트 정리, 신뢰성 캘리브레이션 검증, 배포 최종 점검 |
...
자세한 결과와 알려진 한계는 [`docs/Result.md`](docs/Result.md), 서비스 기획 전체 내용은 [`docs/Service기획서.md`](docs/Service기획서.md)를 참고하세요.
