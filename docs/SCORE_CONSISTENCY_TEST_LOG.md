...
**결정**: `GEMINI_TEMPERATURE=0.15`를 유지하고, temperature 추가 조정은
중단한다. 대신 스펙 2순위 원인(촬영 조건 자체의 편차)에 대응하는
실시간 촬영 가이드(밝기·거리 안내, 이미 구현 완료)가 실사용 환경에서는
더 실질적인 효과를 낼 가능성이 높다 — 동일 사진 재요청 테스트는 촬영
조건 변수를 원천 제거한 "모델 자체의 편차"만 보여주므로, 실사용
